...
if(-not $downloaded){ Write-Host '下载失败，hosts 未修改。' -ForegroundColor Red; exit 1 }

# --- 2. 测速多个 github.com 候选 IP，选最快的 ---
Write-Host '正在测速 github.com 候选 IP...'
...
foreach($ip in $candidates){
  try{
    ...
    if($code -eq '200' -and $sw.Elapsed.TotalSeconds -lt $bestTime){
      ...
      Write-Host ('  ' + $ip + ' -> HTTP ' + $code + ', ' + [int]$sw.Elapsed.TotalMilliseconds + 'ms  [当前最快]')
    } else {
...
if($best){
  Write-Host ('选中 github.com IP: ' + $best + ' (' + [int]($bestTime*1000) + 'ms)')
  # 把下载内容里的 github.com 行替换为测速选出的 IP
  ...
  Write-Host '所有候选 IP 均不通，使用下载的默认记录。' -ForegroundColor Yellow
}
...
Write-Host '智能更新完成，DNS 已刷新。' -ForegroundColor Green
"
echo.
echo 按任意键退出。
pause >nul
